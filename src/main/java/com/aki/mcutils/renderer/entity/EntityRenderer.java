...
import com.aki.mcutils.renderer.integrate.FairyLights;
...
public class EntityRenderer {
    ...
    private boolean shouldRender(Entity entity, ICamera frustum, double partialTicks, double camX, double camY, double camZ) {
        ...
        if (!((IEntityRendererCache) entity).isChunkLoaded()) {
            return false;
        }
        ...
        if (!((IEntityRendererCache) entity).getRenderer().shouldRender(entity, frustum, camX, camY, camZ) && !entity.isRidingOrBeingRiddenBy(Minecraft.getMinecraft().player) && (!MCUtils.isFairyLightsInstalled || !FairyLights.isFairyLightEntity(entity))) {
