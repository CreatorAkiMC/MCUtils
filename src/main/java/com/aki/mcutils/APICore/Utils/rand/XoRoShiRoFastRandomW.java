package com.aki.mcutils.APICore.Utils.rand;

import java.util.Random;

public class XoRoShiRoFastRandomW extends Random {
    private static final long serialVersionUID = 1L;

    private SplitMixRand mixer;
    ...
    public static long randomSeed() {
        ...
        synchronized (XoRoShiRoFastRandomW.seedUniquifier) {
            x = XoRoShiRoFastRandomW.seedUniquifier.nextLong();
        }

        return x ^ System.nanoTime();
    }

    public XoRoShiRoFastRandomW() {
        this(XoRoShiRoFastRandomW.randomSeed());
    }

    public XoRoShiRoFastRandomW(final long seed) {
        this.setSeed(seed);
    }
    ...
    public void setSeed(final long seed) {
        ...
        if (this.hasSavedState && this.seed == seed) {
            ...
            SplitMixRand mixer = this.mixer;

            // Avoid allocations of SplitMixRandom
            if (mixer == null) {
                mixer = this.mixer = new SplitMixRand(seed);
            } else {
                mixer.setSeed(seed);
            }
    ...
    public XoRoShiRoFastRandomW setSeedAndReturn(final long seed) {
        this.setSeed(seed);
        return this;
    }
}
