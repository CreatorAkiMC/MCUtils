...
public class GFastList<T> extends AbstractList<T> implements List<T>, Cloneable, Serializable {
    ...
    public GFastList(int initialCapacity) {
    ...
    public GFastList(Collection<? extends T> c) {
    ...
    public void clear() {
        ...
        int capacity = Math.min(MAX_ARRAY_SIZE, Math.max(size, Math.max(initialCapacity, DEFAULT_CAPACITY)));
    ...
    private class Itr implements Iterator<T> {
        ...
        public void remove() {
            ...
            try {
                GFastList.this.remove(lastReturn);
    ...
    private class ListItr extends Itr implements ListIterator<T> {
        ...
        public void set(T t) {
            ...
            try {
                GFastList.this.set(lastReturn, t);
            } catch (IndexOutOfBoundsException e) {
                throw new ConcurrentModificationException();
            }
        }
    ...
    public Object clone() {
        ...
        try {
            GFastList<T> clone = (GFastList<T>) super.clone();

            clone.first = clone.last = null;

            int capacity = Math.min(MAX_ARRAY_SIZE, Math.max(clone.size, Math.max(clone.initialCapacity, DEFAULT_CAPACITY)));
